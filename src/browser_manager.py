...
class BrowserManager:
    ...
    async def get_tab(self, instance_id: str) -> Optional[Tab]:
        ...
        Also updates last_activity so the idle-timeout tracker knows the instance is in use.
    ...
    async def cleanup_inactive(self, timeout_minutes: int = 5) -> List[str]:
        """
        Close browser instances that have been idle longer than *timeout_minutes*.

        Returns:
            List[str]: IDs of instances that were closed.
        """
        now = datetime.now()
        ...
        for instance_id in to_close:
            debug_logger.log_info(
                ...
                f"Auto-closing idle instance {instance_id}"
            )
