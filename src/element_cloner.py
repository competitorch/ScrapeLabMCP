...
class ElementCloner:
    ...
    async def _evaluate_js_extraction(self, tab, js_filename: str, selector: str, options: dict, method_name: str) -> Dict[str, Any]:
        """Common pattern for JS-based extraction: load file, evaluate, convert result.

        Args:
            tab: Browser tab instance.
            js_filename: Name of the JS file under src/js/.
            selector: CSS selector for the target element.
            options: Options dict passed to the JS template.
            method_name: Name for debug logging.

        Returns:
            Dict[str, Any]: Extraction result or error dict.
        """
        if not selector:
            return {"error": "Selector is required"}
        js_code = self._load_js_file(js_filename, selector, options)
        data = await tab.evaluate(js_code)
        if hasattr(data, 'exception_details'):
            return {"error": f"JavaScript error: {data.exception_details}"}
        if isinstance(data, list):
            data = self._convert_nodriver_result(data)
        if isinstance(data, dict):
            debug_logger.log_info("element_cloner", method_name, f"Extracted data for element")
            return data
        debug_logger.log_warning("element_cloner", method_name, f"Got unexpected type: {type(data)}")
        return {"error": f"Unexpected return type: {type(data)}", "raw_data": str(data)}
    ...
    async def extract_element_structure(
        ...
        """
        try:
            return await self._evaluate_js_extraction(tab, 'extract_structure.js', selector, {
    ...
    async def extract_element_events(
        ...
        """
        try:
            return await self._evaluate_js_extraction(tab, 'extract_events.js', selector, {
    ...
    async def extract_element_animations(
        ...
        """
        try:
            return await self._evaluate_js_extraction(tab, 'extract_animations.js', selector, {
