...
async def app_lifespan(server):
    ...
    # Initialize scraping subsystem
    if is_section_enabled("scraping"):
        try:
            await init_db()
            debug_logger.log_info("server", "startup", "Recipe DB initialized")
        except Exception as e:
            debug_logger.log_error("server", "startup", f"Recipe DB init failed: {e}")
    ...
    try:
        ...
        # Close scraping subsystem
        if is_section_enabled("scraping"):
            try:
                await close_db()
                debug_logger.log_info("server", "cleanup", "Recipe DB closed")
            except Exception as e:
                debug_logger.log_error("server", "cleanup", f"Recipe DB close failed: {e}")
...
async def scrape_url(url: str, headless: bool = True) -> Dict[str, Any]:
    ...
    if recipe and recipe.get("id"):
        await increment_usage(recipe["id"])
    result = await scrape_smart(url, browser_manager, recipe=recipe, headless=headless)
    return result


@section_tool("scraping")
async def discover_url(url: str) -> Dict[str, Any]:
    """
    DISCOVERY PROCESS: Analyze a URL to understand its structure before scraping.
    Returns page analysis with preview data. If a recipe exists for this site,
    returns the recipe instead. Always use this BEFORE scrape_url for unknown sites.

    Args:
        url (str): The URL to discover/analyze.

    Returns:
        Dict[str, Any]: Recipe (if exists) or page analysis with framework detection, JSON-LD, API endpoints.
    """
    recipe = await match_recipe(url)
    if recipe:
        return {
            "has_recipe": True,
            "recipe": recipe,
            "message": "Recipe found for this site. Use scrape_url to scrape with this recipe.",
        }

    # No recipe — analyze with HTTP first, browser fallback
    result = await scrape_smart(url, browser_manager, recipe=None, headless=True)
    result["has_recipe"] = False
    result["message"] = (
        "No recipe for this site. Review the analysis and consider saving a recipe "
        "with save_recipe for faster future scraping."
    )
    return result
...
async def save_recipe(
    ...
    Args:
        ...
        site_name (str): Human-readable site name (e.g. 'WeRoad').
        scrape_level (int): 1=HTTP, 2=Browser stealth. Default: 2.
        wait_for (Optional[str]): CSS selector to wait for before page is ready.
        needs_proxy (bool): Whether proxy rotation is needed.
...
async def delete_recipe(recipe_id: int) -> Dict[str, Any]:
    """
    Delete a scraping recipe by ID.
    ...
    """
    deleted = await db_delete_recipe(recipe_id)
    if deleted:
...
async def list_recipes() -> Dict[str, Any]:
    """
    List all saved scraping recipes and available knowledge (specialists + schemas).

    Returns:
        Dict[str, Any]: Recipes list and knowledge index.
    """
    ...
    return {
        ...
        "knowledge": {
            ...
            "agent_workflow_available": bool(agent_workflow),
        },
    }


@section_tool("scraping")
def get_specialist_recipe(recipe: str) -> Dict[str, Any]:
    """
    Get specialist scraping recipe for a technology
    (api-rest, api-graphql, dom-playwright, ecommerce, frameworks, wordpress).

    Args:
        recipe (str): Specialist name. One of: api-rest, api-graphql, dom-playwright, ecommerce, frameworks, wordpress.

    Returns:
        Dict[str, Any]: Specialist markdown content and description.
    """
    if recipe not in specialists:
        return {"error": f"Unknown specialist '{recipe}'. Available: {VALID_SPECIALISTS}"}
    return specialists[recipe]


@section_tool("scraping")
def get_output_schema(schema_type: str) -> Dict[str, Any]:
    """
    Get JSON output schema (generic, travel, ecommerce).

    Args:
        schema_type (str): Schema type. One of: generic, travel, ecommerce.

    Returns:
        Dict[str, Any]: JSON schema content.
    """
    if schema_type not in schemas:
        return {"error": f"Unknown schema '{schema_type}'. Available: {VALID_SCHEMAS}"}
    try:
        return json.loads(schemas[schema_type])
    except json.JSONDecodeError:
        return {"raw": schemas[schema_type]}
...
async def batch_scrape_urls(
    ...
    Args:
        ...
        recipe_id (Optional[int]): Recipe ID to use (from list_recipes).
        headless (bool): Run browser in headless mode (default True).
...
@section_tool("scraping")
def get_scraping_workflow() -> Dict[str, Any]:
    """
    Get the agent workflow instructions for scraping unknown sites.
    Returns the full agent.md prompt that guides the scraping process.

    Returns:
        Dict[str, Any]: Agent workflow markdown content.
    """
    return {
        "workflow": agent_workflow or "No agent workflow found. Check prompts/agent.md.",
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--disable-scraping", action="store_true",
                      help="Disable scraping intelligence tools (recipes, smart scrape, batch)")
    ...
    if args.list_sections:
        ...
        print("  scraping: Scraping intelligence with recipes and smart routing (9 tools)")
    ...
    if args.disable_scraping:
        DISABLED_SECTIONS.add("scraping")
