...
# ── Markdown engine ───────────────────────────────────────────────────
# Two converters: one preserves links (full mode), one is compact (readability).
...
_JUNK_TAGS = ("script", "style", "svg", "noscript")
_CHROME_TAGS = ("nav", "footer", "header", "aside")  # layout chrome

# Selectors for elements that are almost never main content
_JUNK_SELECTOR_PATTERNS = (
    r'<[^>]+(?:class|id)\s*=\s*"[^"]*(?:cookie|consent|banner|gdpr|popup|overlay|modal|'
    r'sidebar|widget|advert|promo|newsletter|social|share|related|trending|recommend)[^"]*"'
    r'[^>]*>[\s\S]*?</\w+>'
)


def _strip_tags(html: str, tags: tuple) -> str:
    for tag in tags:
        html = re.sub(rf"<{tag}[\s\S]*?</{tag}>", "", html, flags=re.I)
    return html


def _strip_junk_by_selector(html: str) -> str:
    """Remove divs/sections with class/id matching common junk patterns."""
    return re.sub(_JUNK_SELECTOR_PATTERNS, "", html, flags=re.I)


def _clean_markdown(md: str) -> str:
    """Post-process: collapse blank lines, strip trailing spaces, remove empty links."""
    md = re.sub(r"\n{3,}", "\n\n", md)           # max 1 blank line
    md = re.sub(r"\[]\(.*?\)", "", md)             # empty links []()
    md = re.sub(r"!\[]\(.*?\)", "", md)            # empty images ![]()
    md = re.sub(r"^ +$", "", md, flags=re.M)      # whitespace-only lines
    return md.strip()


def _html_to_markdown(html: str, readability: bool = False, url: str = "") -> str:
    """
    Convert HTML to LLM-optimised Markdown.

    readability=False  → strip junk + layout chrome, convert full page via html2text
    readability=True   → trafilatura extracts main content → clean markdown (best for articles/pages)
    """
    if readability:
        ...
        # Fallback to full mode if trafilatura extracts too little
        # (e.g. SPAs with minimal server-rendered content)

    # Full mode: strip junk + chrome, convert with html2text
    cleaned = _strip_tags(html, _JUNK_TAGS)
    cleaned = re.sub(r"<!--[\s\S]*?-->", "", cleaned)
    cleaned = _strip_tags(cleaned, _CHROME_TAGS)
    cleaned = _strip_junk_by_selector(cleaned)
    md = _h2t.handle(cleaned)
    return _clean_markdown(md)
...
class DOMHandler:
    ...
    async def get_page_content(
        ...
        Args:
            tab (Tab): The browser tab object.
            content_type (str): 'html', 'text', or 'markdown'.
            readability (bool): Extract main article content only (Mozilla Readability).
        ...
        try:
            ...
            if content_type == "markdown":
                markdown = _html_to_markdown(html, readability=readability, url=url)
                return {
                    'markdown': markdown,
                    'url': url,
                    'title': title,
